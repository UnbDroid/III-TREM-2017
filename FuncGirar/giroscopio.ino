/* Essas são as funções relacionadas com o giroscopio e com a obtenção do ângulo do robô. Eu sinceramente não faço ideia do que acontece aqui, o que é necessário saber:
    - Chame a função "setupGyro()" uma vez para configurar o sensor;
    - Chame as funções "calibrateGyro()" para calibrar, e "updateGyroValues()" e "updateHeadings()" são chamadas pra atualizar o valor do angulo;
    - A variável[2] possui a posição angular atual (é atualizada pelas funções acima);
*/
