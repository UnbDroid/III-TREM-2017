...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(Encoder1), FazCoisa, RISING);         //Aqui se define qual é o pino responsável pelo interrupt
  //(usado na função "digitalPinToInterrupt(PINO)"), qual o nome da função (ela não recebe nenhum parâmetro), e quando 
  //que a função é chamada ("RISING" para quando o sinal passa de 0 para 1, "CHANGE" quando vai de 0 para 1 ou de 1
  //para 0, as demais classificações vc procura)
